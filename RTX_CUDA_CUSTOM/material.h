...
public:
    __device__ lambertian() : albedo(0) {}
    __device__ lambertian(const vec3& a) : albedo(new solid_color(a)) {}
    __device__ lambertian(tex* a) : albedo(a) {}
...
public:
    ...
    __device__ virtual bool scatter(const ray& r_in,
        ...
        // This calculate the cosine for Schlick Approximation
        ...
        // Check the surface reflection probaility
        if (refract(r_in.direction(), outward_normal, ni_over_nt, refracted))
            reflect_prob = schlick(cosine, ref_idx);
        else
            reflect_prob = 1.0f;

        // Pseudo random between reflection and refraction
...
public:
    ...
    __device__ diffuse_light(const vec3& c) : emit(new solid_color(c)) {}
...
public:
    __device__ isotropic(vec3 c) : albedo(new solid_color(c)) {}
    __device__ isotropic(tex* a) : albedo(a) {}

    __device__ virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered, curandState* local_rand_state
    ) const override {
